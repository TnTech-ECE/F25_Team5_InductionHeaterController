...
3. [Detailed Design](#3-detailed-design)
     ...
     [a. Power Subsystem](#a-power-subsystem)

   
     [b. Safety and Protections Subsystem](#b-detailed-design-safety-and-protections-subsystem)

   
     [c. PCB Subsystem](#c-detailed-design-pcb-subsystem)

   
     [d. Heat Generation Subsystem](#d-detailed-design-heat-generation-subsystem)

   
     [e. Embedded Subsystem](#e-detailed-design-embedded-susbystem)
...
![alt text](<Power System/Screenshot 2025-11-27 163138.png>)
        **Figure 8, No smoothing Capacitor on DC mains.
![alt text](<Power System/Screenshot 2025-11-27 163253.png>) 
...
## b. Detailed Design Safety and Protections Subsystem 
...
<div class="table-container table-bom">
...
## c. Detailed Design PCB Subsystem
...
![block_diagram](<images_PCB/component_3d_Model.jpg>)
...
![Terminal IO shield board schematic](<images_PCB/terminal_IO_schematic.jpg>)
...
![AC conversion schematic](<images_PCB/AC.jpg>)
AC conversion schematic

![DC schematic part 1](<images_PCB/DC_1.jpg>)
DC schematic part 1

![DC schematic part 2](<images_PCB/DC_2.jpg>)
DC schematic part 2

![Safety schematic part 1](<images_PCB/Safety_1.jpg>)



Safety schematic part 1

![Safety schematic part 2](<images_PCB/Safety_2.jpg>)

Safety schematic part 2

![Safety schematic part 3](<images_PCB/Safety_3.jpg>)

Safety schematic part 3

![Gate Driver 1](<images_PCB/gate_driver_1.jpg>)
Gate Driver 1

![Gate Driver 2](<images_PCB/gate_driver_2.jpg>)
Gate Driver 2

![Gate Driver 3](<images_PCB/gate_driver_3.jpg>)
Gate Driver 3

![Gate Driver 4](<images_PCB/gate_driver_4.jpg>)
Gate Driver 4

![Root Schematic part 1](<images_PCB/root_Sch_1.jpg>)

Root Schematic part 1

![Root Schematic part 2](<images_PCB/root_Sch_2.jpg>)
Root Schematic part 2

![Root Schematic part 3](<images_PCB/root_Sch_3.jpg>)
Root Schematic part 3

![Root Schematic part 4](<images_PCB/root_Sch_4.jpg>)
Root Schematic part 4
### Printed Circuit Board Layout

![Power PCB board](<images_PCB/power%20PCB%20board%20screenshot.jpg>)

Power PCB board

![Power PCB board with routes ](<images_PCB/power%20PCB%20board%20routes%20screenshot.jpg>)

Power PCB board with Routes

![IO shield Board ](<images_PCB/IO%20shield%20Board%20screenshot.jpg>)

IO shield Board

![IO shield Board with Routes ](<images_PCB/IO%20shield%20Board%20routes%20screenshot.jpg>)

IO shield Board with Routes

### Flowchart
![block_diagram](<images_PCB/block%20diagram.jpg>)
...
## d. Detailed Design Heat Generation Subsystem 
...
## e. Detailed Design Embedded Susbystem 
...
### Experiment 2 - Power Subsystem: Half-Bridge Inverter Verification
  ...
  ![alt text](<26a761b0-5939-4a0a-883a-dddb5e5a429b (3).jpg>)
...
For this project's future goals, it should make a transition from prototyping to actual testing. This would include scaling the power as well as PCB optimization. One primary objective is to move from the current 30 VDC power supply used during the experimental phase to a full 120 VAC supply, which will allow the team to perform full-load testing and accurately measure the actual thermal rise of the metal pipe workpiece. This is essential to verify how the system handles higher voltage levels and to observe the impact on temperature response times. Additionally, future teams should aim to design and refine PID control logic by conducting experiments that directly correlate full-load power input with thermal data, ensuring more precise and stable temperature regulation. Future work should also focus on hardware refinement, specifically iterating on the PCB design to correct trace routing issues and improve the layering of surface mount components for a more professional and efficient final product. Likewise, the Rutland Gasket Cement was discovered not to be as adhesive as anticipated, so future work may prefer to use high heat epoxy since it was decided it is a good idea to stay in low temperature ranges (less than 100 °C) to avoid boiling water. Ceramic cement tends to be less sticky than epoxy, so it is harder to work with, but is still an option if there is a fear of exceeding 300 °F. The induction coil acted as a spring after being wrapped around the pipe (due to the 10 gauge wire's insulation) so getting a proper amount of adhesive to hold the coil together is just as important as properly mounting the thermocouple to the pipe. 

# 6. Final Conclusion

In conclusion, the project successfully established a framework for contactless induction heating through the integration of power electronics, safety subsystems, and embedded controls. The prototype demonstrated that it is capable of inducing surface eddy currents and internal heating while providing accurate real-time data via user interface. While purchasing delays made it necessary to move to a half-bridge inverter for initial testing, the experimental results confirmed that the gate driver circuits and hardware safety latches function as intended. Ultimately, this prototype serves as a successful proof-of-concept for Lochinvar, meeting the core functional requirements and setting a clear path for the development of high-efficiency induction heating solutions.
